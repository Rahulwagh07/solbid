...
export async function GET(req: Request) {
  try {
    ...
    const highestBid = Math.max(
      ...user.players.map((player) => player.totalBidAmount),
    );
    const totalRoyalties = user.players.reduce(
      (acc, player) => acc + player.royaltyEarned,
      0,
    );
    const totalAmount = user.players.reduce(
      (acc, player) => acc + player.totalBidAmount,
      0,
    );
