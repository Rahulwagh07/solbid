...
export default function PastGames({ games }: { games: PastGame[] }) {
  return (
    <>
      <h2 className="text-lg sm:text-3xl font-bold mb-6">Past Games</h2>
        ...
        {games.map((game) => (
          <div key={game.gameId} className="flex flex-col gap-4 sm:gap-0  text-sm sm:flex-row items-start sm:items-center justify-between p-4 mb-4  border border-slate-700/90 bg-slate-700/50 rounded-lg shadow">
            <div>
              ...
              <p className="text-sm text-slate-400">Ended: {new Date(game.endTime).toLocaleString()}</p>
            </div>
            <div className="sm:text-right">
              <p className="font-medium">Total Bid: ${game.totalAmount}</p>
              <p className="text-sm text-slate-400">Royalty Earned: ${game.totalRoyalty}</p>
            </div>
            <Link href={`/game/${game.gameId}`} className='bg-white text-black py-1.5 px-4 rounded-md border'>
              View
            </Link>
          </div>
        ))}
