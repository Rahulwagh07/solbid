...
export const createGame = async (
  ...
  return {
    instructions: [createGameIx],
    totalCost: initialBidLamports,
    gamePda,
    playerPda,
    bidPda,
  };
...
export const getAllPlayersAndBids = async (
  gameId: number,
...
export const getAllPlayersAndBidsForGame = async (gameId: number) => {
  try {
    ...
    for (const player of players) {
      const { playerPda, playerId }: { playerPda: string; playerId: number } =
        player;
      const accountInfo = await CONNECTION.getAccountInfo(
        new PublicKey(playerPda),
      );
      if (accountInfo && accountInfo.data) {
        ...
        if (playerState) {
          playerStates.push({
            ...
            royalty_earned: convertLamportsToUsdc(
              parseInt(playerState.royalty_earned.toString()),
            ),
