...
export const placeBid = async (
  ...
  playersPubkeys.forEach((playerPubkey: string) => {
    keys.push({
      pubkey: new PublicKey(playerPubkey),
      isSigner: false,
      isWritable: true,
    });
  });
  ...
  return {
    ...
    totalCost: bidAmountLamports,
    playerPda,
    bidPda,
  };
};
export const placeBidScale = async (
  bidder: PublicKey,
  gameId: number,
  bidAmount: number,
  bidCount: number,
  ...
  const playersPubkeys = res.data.playersPubkey.map(
