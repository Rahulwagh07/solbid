...
export function useTransaction(connection: Connection) {
  ...
  const resetStatus = () =>
    setStatus({ state: "idle", message: STATUS_MESSAGES.idle });

  const execute = useCallback(
    async (
      createInstructions: () => Promise<{
      ...
      try {
        ...
        if (balance < (totalCost || 0)) {
          throw new Error(
            `Insufficient balance. You need at least ${(totalCost || 0) / LAMPORTS_PER_SOL} SOL`,
          );
        }

        setStatus({
          state: "awaitingApproval",
          message: STATUS_MESSAGES.awaitingApproval,
        });
